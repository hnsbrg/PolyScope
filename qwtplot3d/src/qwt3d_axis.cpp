...
void Axis::drawLabel()
{
    ...
    if ( ( beg.z - end.z < TOLERANCE * length() ) && ( ( beg.x - end.x )  < TOLERANCE * length() || ( beg.y - end.y ) < TOLERANCE * length() ) )
    {
        if ( CenterRight == scaleNumberAnchor_ )
        {
            // a vertical axis
            angle = 90;
        }

        if ( TopCenter == scaleNumberAnchor_ )
        {
            // a horizontal axis
            angle = 0;
        }
