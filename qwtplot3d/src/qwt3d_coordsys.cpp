...
const double FACE_TIC_LENGTH = 0.02; //  in proportion of the scale
...
void CoordinateSystem::chooseAxes()
{
    ...
    // WDH 1.19.2017
    // special cases -   are we looking directly at one of the six faces?
    bool show_face = false;

    const double TOLERANCE = 0.001;

    double tolerance = axes[Y1].length() * TOLERANCE;

    // this corresponds to testing that the face is parallel to the screen to
    // a tolerance of 0.057 degree
    // eg. the sin angle == (beg[X1].x - beg[X4].x) / length between X1 and X4 axes

    if ( qAbs( beg[X1].x - beg[X4].x ) < tolerance &&
         qAbs( beg[X1].y - beg[X4].y ) < tolerance  &&
         qAbs( end[X1].x - end[X4].x ) < tolerance &&
         qAbs( end[X1].y - end[X4].y ) < tolerance )
    {
        // looking at left or right face
        if ( beg[X1].z > beg[X4].z )
        {
            show_face = true;

            double tic_length = FACE_TIC_LENGTH * axes[Z3].length();
            axes[X4].setTicLength( tic_length, 0.0 );

            axes[X4].setTicOrientation( 0, 0, -1 );  //0 1 0
            axes[X4].setScaling( true );
            axes[X4].setNumbers( true );
            axes[X4].setLabel( true );
            axes[X4].setNumberAnchor( TopCenter );

            tic_length = FACE_TIC_LENGTH * axes[X4].length();
            axes[Z3].setTicLength( tic_length, 0.0 );

            axes[Z3].setTicOrientation( 1, 0, 0 );
            axes[Z3].setScaling( true );
            axes[Z3].setNumbers( true );
            axes[Z3].setLabel( true );
            axes[Z3].setNumberAnchor( CenterRight );
        }
        else
        {
            show_face = true;

            double tic_length = FACE_TIC_LENGTH * axes[Z1].length();
            axes[X1].setTicLength( tic_length, 0.0 );

            axes[X1].setTicOrientation( 0, 0, -1 );  // 0 -1 0
            axes[X1].setScaling( true );
            axes[X1].setNumbers( true );
            axes[X1].setLabel( true );
            axes[X1].setNumberAnchor( TopCenter );

            tic_length = FACE_TIC_LENGTH * axes[X1].length();
            axes[Z1].setTicLength( tic_length, 0.0 );

            axes[Z1].setTicOrientation( -1, 0, 0 );
            axes[Z1].setScaling( true );
            axes[Z1].setNumbers( true );
            axes[Z1].setLabel( true );
            axes[Z1].setNumberAnchor( CenterRight );
        }
    }
    else
    {
        tolerance = axes[Z1].length() * TOLERANCE;

        if ( qAbs( beg[X1].x - beg[X2].x ) < tolerance &&
             qAbs( beg[X1].y - beg[X2].y ) < tolerance  &&
             qAbs( end[X1].x - end[X2].x ) < tolerance &&
             qAbs( end[X1].y - end[X2].y ) < tolerance )
        {
            // looking at top or bottom face
            if ( beg[X1].z > beg[X2].z )
            {
                show_face = true;

                double tic_length = FACE_TIC_LENGTH * axes[Y3].length();
                axes[X2].setTicLength( tic_length, 0.0 );

                axes[X2].setTicOrientation( 0, -1, 0 );
                axes[X2].setScaling( true );
                axes[X2].setNumbers( true );
                axes[X2].setLabel( true );
                axes[X2].setNumberAnchor( CenterRight );

                tic_length = FACE_TIC_LENGTH * axes[X2].length();
                axes[Y3].setTicLength( tic_length, 0.0 );

                axes[Y3].setTicOrientation( 1, 0, 0 );
                axes[Y3].setScaling( true );
                axes[Y3].setNumbers( true );
                axes[Y3].setLabel( true );
                axes[Y3].setNumberAnchor( TopCenter );
            }
            else
            {
                show_face = true;

                double tic_length = FACE_TIC_LENGTH * axes[Y1].length();
                axes[X1].setTicLength( tic_length, 0.0 );
                ...
                axes[X1].setNumbers( true );
                axes[X1].setLabel( true );
                axes[X1].setNumberAnchor( CenterRight );

                tic_length = FACE_TIC_LENGTH * axes[X1].length();
                axes[Y1].setTicLength( tic_length, 0.0 );

                axes[Y1].setTicOrientation( -1, 0, 0 );
                axes[Y1].setScaling( true );
                axes[Y1].setNumbers( true );
                axes[Y1].setLabel( true );
                axes[Y1].setNumberAnchor( TopCenter );
            }
        }
        else
        {
            tolerance = axes[X1].length() * TOLERANCE;

            if ( qAbs( beg[Z1].x - beg[Z4].x ) < tolerance &&
                 qAbs( beg[Z1].y - beg[Z4].y ) < tolerance  &&
                 qAbs( end[Z1].x - end[Z4].x ) < tolerance &&
                 qAbs( end[Z1].y - end[Z4].y ) < tolerance )
            {
                // looking at from or rear face
                if ( beg[Z1].z > beg[Z4].z )
                {
                    show_face = true;

                    double tic_length = FACE_TIC_LENGTH * axes[Y2].length();
                    axes[Z2].setTicLength( tic_length, 0.0 );

                    axes[Z2].setTicOrientation( 0, -1, 0 ); // -1 0 0
                    axes[Z2].setScaling( true );
                    axes[Z2].setNumbers( true );
                    axes[Z2].setLabel( true );
                    axes[Z2].setNumberAnchor( CenterRight );

                    tic_length = FACE_TIC_LENGTH * axes[Z2].length();
                    axes[Y2].setTicLength( tic_length, 0.0 );

                    axes[Y2].setTicOrientation( 0, 0, -1 ); // 1 0 0
                    axes[Y2].setScaling( true );
                    axes[Y2].setNumbers( true );
                    axes[Y2].setLabel( true );
                    axes[Y2].setNumberAnchor( TopCenter );
                }
                else
                {
                    show_face = true;
                    ...
                    double tic_length = FACE_TIC_LENGTH * axes[Y1].length();
                    axes[Z4].setTicLength( tic_length, 0.0 );

                    axes[Z4].setTicOrientation( 0, 1, 0 );
                    axes[Z4].setScaling( true );
                    axes[Z4].setNumbers( true );
                    axes[Z4].setLabel( true );
                    axes[Z4].setNumberAnchor( CenterRight );

                    tic_length = FACE_TIC_LENGTH * axes[Z4].length();
                    axes[Y1].setTicLength( tic_length, 0.0 );

                    axes[Y1].setTicOrientation( 0, 0, -1 );
                    axes[Y1].setScaling( true );
                    axes[Y1].setNumbers( true );
                    axes[Y1].setLabel( true );
                    axes[Y1].setNumberAnchor( TopCenter );
    ...
    // leave if we have found we are looking at a face
    if ( true == show_face )
    {
        return;
    }

    // otherwise continue to evaluate the general case
