...
QImage Label::createImage( double angle )
{
    ...
    QImage img = pm_.toImage().convertToFormat(QImage::Format_ARGB32);
...
QImage Label::createMultiLineImage( double angle )
{
    int max_line_ht = 0;
    int max_line_width = 0;
    int max_line_chars = 0;

    QStringList list = text_.split( QChar( '\n' ) );

    for ( int i = 0; i < list.count(); i++ )
    {
        QRect r = QRect( QPoint( 0, 0 ), QFontMetrics( font_ ).size( Qwt3D::SingleLine, list.at( i ) ) );

        if ( r.height() > max_line_ht )
        {
            max_line_ht = r.height();
        }

        if ( r.width() > max_line_width )
        {
            max_line_width = r.width();
        }

        if ( list.at( i ).length() > max_line_chars )
        {
            max_line_chars = list.at( i ).length();
        }
    }

    int textWidth = max_line_width;
    int textHeight = max_line_ht * list.count();
    ...
    if ( angle >= 270 )
        ...
        p.translate( 0.0, textWidth * sin( rad ) );
    p.rotate( -angle );
    p.translate( 0.0, max_line_ht - QFontMetrics( font_ ).descent() );
    ...
    for ( int i = 0; i < list.count(); i++ )
    {
        ...
        int leading_spaces = max_line_chars - s.length();

        if ( leading_spaces > 0 )
        {
            s.prepend( QString( leading_spaces, QChar( ' ' ) ) );
        }

        p.drawText( 0, i * max_line_ht, s );
    }
...
void Label::drawMultiLine( double angle )
{
    // WDH TODO - make work with multiline text
    ...
    QImage tex_ = createMultiLineImage( angle );
    glDrawPixels( tex_.width(), tex_.height(), GL_RGBA, GL_UNSIGNED_BYTE, tex_.bits() );
    glAlphaFunc( func, v );
    Enable( GL_ALPHA_TEST, b );
}
